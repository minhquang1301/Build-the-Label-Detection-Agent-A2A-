...
def process_shipping_label(pdf_content):
    prompt = f"""Extract the following from this shipping label:
    - Tracking Number
    - Sender Address
    - Recipient Address
    - Delivery Date

    Return the result as a JSON object with clear field names.
    """
    ...
    try:
        ...
        if response.parts:
            ...
            print(response.parts[0].text)  # 👈 IN RA PHẢN HỒI
            return json.loads(response.parts[0].text)
        else:
            print("No parts in Gemini response.")
            return None
    except Exception as e:
        print(f"Error during Gemini data extraction for shipping label: {e}")
        return None
